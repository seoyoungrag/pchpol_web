...
TrexConfig.addAttacher(
	...
	{	
		...
		wysiwygonly: _TRUE,
		objattr: {},
		features: { left:250, top:65, width:807, height:644, resizable:"no" },
...
Trex.Attachment.Image = Trex.Class.create({
	...
	getSaveHtml: function(data) {
		if (data.width && data.height) {
			return "<img src=\"" + data.imageurl + "\" width=\"" + data.width + "\" height=\"" + data.height + "\" class=\"txc-image\"/>";
		} else {
	...
	getDispHtml: function(data) {
		if (data.width && data.height) {
			return "<img id=\"" + data.dispElId + "\" src=\"" + data.imageurl + "\" width=\"" + data.width + "\" height=\"" + data.height + "\" class=\"txc-image\"/>";
		} else {
